...
const logger = bunyan.createLogger({
  ...
  streams: [{
    level: 'info',
    path: `${process.env.LOG_FOLDER}/yad2ScannerFull.log`,
  }],
});
...
async function scanYad2() {
  logger.info('Yad2 full scan started');
  console.log(`🚀  Yad2 full scan started, for more info see log file. (Start time: ${moment().toDate()})`);
  ...
  logger.info(`Yad2 full scan - last scan time: ${lastScanTime}`);

  await scanAds();

  database.disconnect();

  console.log(`👍  Yad2 full scan completed. (End time: ${moment().toDate()})`);
  logger.info('Yad2 full scan completed');
}
